...
<div class="lg:pb-10 min-h-screen dark:bg-gray-900 relative z-0">
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        {{-- Header --}}
        <x-page-header title="Laporan" subtitle="Ringkasan stok, transaksi, dan aktivitas pengguna" />

        {{-- FILTER PANEL --}}
        <x-form.report-filter :filters="$filters" :categories="$categories" :action="route('reports.index')" />
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <x-table.data-table
                ...
                colSpan="col-span-12"
                height="h-[400px]"
                sortColumn="{{ $sortTrxColumn }}"
                sortDirection="{{ $sortTrxDirection }}"
                sortParam="sort_trx"
                directionParam="direction_trx"
                :searchable="true"
                searchPlaceholder="Cari transaksi..."
                currentSearch="{{ $searchTrx }}"
                searchParam="search_trx">
                ...
                <tr>
                    <td colspan="{{ count($trxHeaders) }}" class="px-4 py-8 text-center text-gray-400">
                        @if($searchTrx)
                        Tidak ada hasil untuk "{{ $searchTrx }}".
                        @else
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 mt-6">
            ...
            <div class="space-y-4">
                ...
                <x-table.user-activity-card :user="$user" />
                @endif
                @empty
