    ...
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 sm:items-center mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola semua pengguna yang memiliki akses ke sistem</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-12 gap-4 lg:gap-6">
            ...
            <div class="col-span-12 bg-white rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 dark:bg-gray-800 grid grid-cols-1">
                <div class="p-5 border-b border-gray-100 dark:border-gray-700 grid grid-cols-2 items-center">
                    <h2 class="font-semibold text-gray-900 dark:text-white">Daftar Pengguna</h2>
                    <a href="{{ route('users.create') }}" class="inline-flex items-center gap-1.5 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition justify-self-end">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $roleBadge }}">
                                        {{ $user->role }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <div class="grid grid-flow-col auto-cols-max items-center justify-center gap-2">
                                        <a href="{{ route('users.edit', $user->id) }}" class="p-1.5 text-yellow-600 hover:bg-yellow-50 rounded-lg transition" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Yakin hapus pengguna ini?')" class="inline">
                                            @csrf
                                            @method('DELETE')
